...
export class FhirResourceMapper {
  constructor(private terminologyService: TerminologyService, private transformEngine: TransformEngine) {}

  async map(rawRecord: RawRecord, config: MappingConfiguration): Promise<MappedEntityResult> {
    ...
    if (fhir.resourceType === 'Patient') {
      ...
      if (Array.isArray(fhir.identifier)) {
        for (const id of fhir.identifier) {
          if (id.system?.includes('nid') || id.system?.includes('sa:nid')) mappedData.nationalId = id.value;
          else if (id.system?.includes('iqama')) mappedData.iqamaNo = id.value;
          else if (id.system?.includes('mrn') || id.system?.includes('hospital-c')) mappedData.mrn = id.value;
        }
      }
      if (Array.isArray(fhir.name) && fhir.name.length > 0) {
        const arName = fhir.name.find((n: any) => /[\u0600-\u06FF]/.test(n.family || ''));
        const enName = fhir.name.find((n: any) => !/[\u0600-\u06FF]/.test(n.family || ''));
        if (arName) { mappedData.familyNameAr = arName.family; mappedData.givenNameAr = Array.isArray(arName.given) ? arName.given.join(' ') : arName.given; }
        if (enName) { mappedData.familyName = enName.family; mappedData.givenName = Array.isArray(enName.given) ? enName.given.join(' ') : enName.given; } else if (arName) { mappedData.familyName = arName.family; mappedData.givenName = Array.isArray(arName.given) ? arName.given.join(' ') : arName.given; }
      }
      mappedData.gender = this.transformEngine.transform('gender_normalize', fhir.gender);
      mappedData.birthDate = fhir.birthDate;
      if (Array.isArray(fhir.telecom) && fhir.telecom[0]) mappedData.phone = fhir.telecom[0].value;
      ...
      mappedData.items = items.map((it: any, idx: number) => { const gross = it.unitPrice?.value || 0; const net = it.net?.value || gross; const copay = gross - net; totalGross += gross; totalNet += net; return { sequence: it.sequence || idx + 1, serviceCode: { sourceCode: it.productOrService?.coding?.[0]?.code, sbsCode: it.productOrService?.coding?.[0]?.code, sbsDisplay: it.productOrService?.coding?.[0]?.display }, serviceName: it.productOrService?.coding?.[0]?.display, quantity: it.quantity?.value || 1, unitPriceSAR: gross, totalGrossSAR: gross, patientCopaySAR: copay, netClaimedSAR: net }; });
      mappedData.totalGrossSAR = totalGross; mappedData.totalPatientCopaySAR = totalGross - totalNet; mappedData.totalInsurerClaimedSAR = totalNet;
      ...
      if (!coding?.code) throw new Error("Missing required field for MedicationRequest: code");
      ...
      if (!coding?.code) throw new Error("Missing required field for Immunization: vaccine code");
      ...
      if (coding) { const resolved = await this.terminologyService.resolveCode(coding.code, coding.system || 'snomed', 'DIAGNOSIS'); mappedData.code = resolved; } else throw new Error("Missing required field for Condition: code");
      ...
      const coding = fhir.code?.coding?.[0];
      if (coding) { const resolved = await this.terminologyService.resolveCode(coding.code, coding.system || 'loinc', 'LAB_TEST'); mappedData.code = resolved; } else throw new Error("Missing required field for Observation: code");
      if (fhir.valueQuantity) mappedData.valueQuantity = { value: fhir.valueQuantity.value, unit: fhir.valueQuantity.unit || '%' };
      ...
      mappedData.recordedDate = fhir.recordedDate || new Date().toISOString();
      const coding = fhir.code?.coding?.[0];
      if (!coding?.code) throw new Error("Missing required field for Allergy: code");
      const resolvedSubstance = await this.terminologyService.resolveCode(coding.code, coding.system || 'snomed', 'ALLERGY' as any);
      ...
      const coding = fhir.code?.coding?.[0];
      if (!coding?.code) throw new Error("Missing required field for DiagnosticReport: code");
