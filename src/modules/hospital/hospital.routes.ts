...
export function createHospitalRoutes(engine: any) {
  const router = Router();
  router.get('/hospitals', verifyToken as any, async (req, res) => {
    try {
  ...
  router.post('/hospitals/onboard', verifyToken as any, requirePermission('ORG_MANAGE_ALL') as any, async (req, res) => {
    try {
      const def = req.body;
      if (!def.hospitalName || !def.hospitalNameAr) return res.status(400).json({ error: 'Hospital names (Ar/En) are required.' });
      ...
      if (!adminUsername || !adminPassword || !adminFullName || !adminEmail || !adminPhoneRaw) return res.status(400).json({ error: 'بيانات أدمن المنشأة مطلوبة: username, password, fullName, email, phone' });
      if (adminPassword.length < 8 || !/[A-Za-z]/.test(adminPassword) || !/\d/.test(adminPassword)) return res.status(400).json({ error: 'كلمة مرور الأدمن ضعيفة: 8+ حروف وأرقام' });
  ...
  router.post('/hospitals/:id/ingest', verifyToken as any, requirePermission('IMPORT_EXECUTE_ORG') as any, async (req, res) => {
    try {
  ...
  router.post('/ingest/file', verifyToken as any, requirePermission('IMPORT_EXECUTE_ORG') as any, async (req, res) => {
    try {
      const { fileName, fileContent, sourceSystemId } = req.body;
      if (!fileName || !fileContent) return res.status(400).json({ error: 'Both fileName and fileContent are required.' });
      ...
      await prisma.auditLog.create({ data: { entity_type: 'DataImport', entity_id: fileName, action: 'FILE_INGEST', actor_id: (req as any).user?.id, organization_id: resolvedSystemId, new_values: JSON.stringify({ fileName, format: (result as any).format }), details: `File ingest ${fileName} via ${resolvedSystemId}` } }).catch(()=>{});
      res.json({ success: true, result });
    } catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
  });
  return router;
