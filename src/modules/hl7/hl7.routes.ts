...
export function createHl7Routes(engine: any) {
  const router = Router();
  router.post('/hl7v2/ingest', verifyToken as any, requirePermission('IMPORT_EXECUTE_ORG') as any, async (req, res) => {
    try {
      const rawHl7 = typeof req.body === 'string' ? req.body : req.body.message || req.body.rawHl7;
      if (!rawHl7) return res.status(400).json({ error: 'HL7 v2 pipe-delimited message is required.' });
