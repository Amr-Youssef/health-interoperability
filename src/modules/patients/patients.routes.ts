...
export function createPatientsRoutes(canonicalStore: any) {
  ...
  router.get('/patients/:id/longitudinal', verifyToken as any, requirePermission('PATIENT_READ_SELF','PATIENT_READ_ORG','PATIENT_READ_ALL','FHIR_READ_SELF','FHIR_READ_ORG','FHIR_READ_ALL') as any, async (req, res) => {
    ...
    if (user && user.role?.role_code === 'MOH_AUDITOR') {
      try { await (prisma as any).auditLog.create({ data: { entity_type: 'Patient', entity_id: requestedId, action: 'AUDIT_VIEW_LONGITUDINAL', actor_id: user.id, organization_id: user.organization_id, details: `MOH_AUDITOR opened longitudinal record in audit context for ${requestedId}` } }).catch(()=>{}); } catch {}
    }
    res.json(record);
  });
  return router;
