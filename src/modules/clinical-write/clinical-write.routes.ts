...
export function createClinicalWriteRoutes(canonicalStore: any) {
  const router = Router();
  router.use(verifyToken);

  const needWrite = requirePermission('CLINICAL_WRITE_ORG') as any;
  const needEncounter = requirePermission('ENCOUNTER_CREATE_ORG') as any;
  ...
  router.post('/encounter', needEncounter, async (req, res) => {
    try {
      ...
      if (!patientId || !encounterClass) return res.status(400).json({ error: 'patientId و encounterClass مطلوبان' });
      const pat = await prisma.patient.findFirst({ where: { OR: [{ id: patientId }, { internal_id: patientId }] } });
      if (!pat) return res.status(404).json({ error: 'المريض غير موجود' });
      ...
      if (!link && !direct) {
        ...
        if (!ok) return res.status(403).json({ error: 'المريض غير مرتبط بمنشأتك ولا يوجد موعد/إذن فعال — احجز موعد أو استخدم كسر الزجاج' });
      }
      const enc = {
      ...
      await canonicalStore.saveEncounter(enc as any);
  ...
  router.post('/condition', needWrite, async (req, res) => {
    try {
      ...
      if (!patientId || !code) return res.status(400).json({ error: 'patientId و code مطلوبان' });
      const pat = await prisma.patient.findFirst({ where: { OR: [{ id: patientId }, { internal_id: patientId }] } });
      if (!pat) return res.status(404).json({ error: 'المريض غير موجود' });
      const cond = {
      ...
      await canonicalStore.saveCondition(cond as any);
  ...
  router.post('/observation', needWrite, async (req, res) => {
    try {
      ...
      if (!patientId || !code || value === undefined) return res.status(400).json({ error: 'patientId و code و value مطلوبة' });
      const pat = await prisma.patient.findFirst({ where: { OR: [{ id: patientId }, { internal_id: patientId }] } });
      if (!pat) return res.status(404).json({ error: 'المريض غير موجود' });
      const obs = {
      ...
      await canonicalStore.saveObservation(obs as any);
  ...
  router.post('/diagnostic-report', needWrite, async (req, res) => {
    try {
      ...
      await canonicalStore.saveDiagnosticReport(rep as any);
  ...
  router.post('/allergy', needWrite, async (req, res) => {
    try {
      ...
      await canonicalStore.saveAllergyIntolerance(alg as any);
  ...
  router.post('/immunization', needWrite, async (req, res) => {
    try {
      ...
      if (!patientId || !vaccineCode) return res.status(400).json({ error: 'patientId و vaccineCode مطلوبان' });
      const pat = await prisma.patient.findFirst({ where: { OR: [{ id: patientId }, { internal_id: patientId }] } });
      if (!pat) return res.status(404).json({ error: 'المريض غير موجود' });
      ...
      await canonicalStore.saveImmunization(imm as any);
  ...
  router.post('/medication', needWrite, async (req, res) => {
    try {
      ...
      await canonicalStore.saveMedicationRequest(med as any);
