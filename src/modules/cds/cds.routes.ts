...
export function createCdsRoutes(engine: any) {
  const router = Router();
  router.get('/cds/patient/:id/safety-alerts', verifyToken as any, requirePermission('CLINICAL_READ_ORG','CLINICAL_READ_ALL') as any, async (req, res) => {
  ...
  router.post('/cds/evaluate-draft-prescription', verifyToken as any, requirePermission('CLINICAL_WRITE_ORG','CLINICAL_READ_ORG') as any, async (req, res) => {
