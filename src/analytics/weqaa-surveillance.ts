...
export interface ReportableDiseaseDefinition {
  ...
  authority: string; // 'Weqaa' (Saudi CDC)
}
...
export const SAUDI_REPORTABLE_DISEASES: ReportableDiseaseDefinition[] = [
  {
    code: 'MERS-COV',
...
export class WeqaaSurveillanceEngine {
  ...
  private dispatchedCases: Map<string, WeqaaReportableCase> = new Map();
   ...
   */
  async generateWeqaaNotificationBundle(caseId: string): Promise<Record<string, any>> {
    const cases = await this.detectReportableCases();
    const targetCase = cases.find(c => c.caseId === caseId) || cases[0];
    if (!targetCase) throw new Error(`Reportable case [${caseId}] not found.`);
   ...
   */
  async dispatchCaseNotification(caseId: string): Promise<WeqaaReportableCase> {
