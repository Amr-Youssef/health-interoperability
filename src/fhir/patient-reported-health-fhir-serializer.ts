...
export class PatientReportedHealthFhirSerializer extends FhirR4Serializer {
  ...
  serializePatientReportedVitalObservation(vital: PatientReportedVitalObservationData): Record<string, any> {
    ...
    return {
      ...
      component:
        vital.observationType === 'BLOOD_PRESSURE' && (vital.systolic || vital.diastolic)
          ? [
              vital.systolic
                ? {
                    code: {
  ...
  // Helper methods
  private mapFamilyRelationshipToFhir(relationship: string): string {
  ...
  private getLoincCodeForObservationType(observationType: string): string {
    const mapping: { [key: string]: string } = {
