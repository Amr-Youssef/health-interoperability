...
router.get('/global-patients', async (req: Request, res: Response) => {
  try {
    if (['HOSPITAL_ADMIN','CLINICIAN'].includes(req.user?.role?.role_code)) return res.status(403).json({ error: 'Global registry hidden for hospital roles — use /patients (org-scoped) only' });
    const q = String(req.query.q || '').trim();
    const page = Math.max(parseInt(String(req.query.page || '1'), 10) || 1, 1);
    const limit = Math.min(Math.max(parseInt(String(req.query.limit || '12'), 10) || 12, 5), 50);
    const skip = (page - 1) * limit;
    const where: any = {};
    if (q) where.OR = [{ internal_id: { contains: q, mode: 'insensitive' } }, { first_name: { contains: q, mode: 'insensitive' } }, { first_name_ar: { contains: q, mode: 'insensitive' } }, { identifiers: { some: { value: { contains: q } } } }];
    ...
    try { await prisma.auditLog.create({ data: { entity_type: 'Patient', entity_id: q || '*', action: 'HOSPITAL_GLOBAL_SEARCH', actor_id: req.user!.id, organization_id: req.user!.organization_id, details: `Global search q="${q}" page=${page} total=${total}` } }).catch(()=>{}); } catch {}
    if (req.query.q || req.query.page) return res.json({ items: mapped, total, page, pageSize: limit, totalPages: Math.ceil(total / limit) });
    res.json(mapped);
  } catch (error) {
...
router.get('/patients', async (req: Request, res: Response) => {
  try {
    ...
    if (req.query.q || req.query.page) return res.json({ items: patients, total, page, pageSize: limit, totalPages: Math.ceil(total/limit) });
    res.json(patients);
  } catch (error) {
...
router.patch('/users/:id', requireHospitalAdmin as any, requirePermission('USER_MANAGE_ORG') as any, async (req, res) => {
  try {
    const orgId = req.user!.organization_id;
    const target = await prisma.user.findUnique({ where: { id: req.params.id as string }, include: { role: true } });
    if (!target || target.organization_id !== orgId) return res.status(404).json({ error: 'User not found in your organization' });
    ...
    const updates: any = {};
    const oldValues: any = {}; const newValues: any = {};
    if (full_name !== undefined) {
      const v=String(full_name).trim();
      if (v.length<3||v.length>80) return res.status(400).json({ error: 'الاسم الكامل بين 3 و 80' });
      if (v.split(/\s+/).length<2) return res.status(400).json({ error: 'الاسم يجب أن يحتوي على الأول والعائلة' });
      oldValues.full_name=target.full_name; newValues.full_name=v; updates.full_name=v;
    }
    if (email !== undefined) {
      const v=String(email).trim().toLowerCase();
      if (v && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)) return res.status(400).json({ error: 'صيغة البريد غير صحيحة' });
      if (v) {
        const dup=await prisma.user.findFirst({ where: { email: v, NOT: { id: target.id } } });
        if (dup) return res.status(400).json({ error: 'البريد مسجل لمستخدم آخر' });
      }
      oldValues.email=target.email; newValues.email=v||null; updates.email=v||null;
    }
    if (phone !== undefined) {
      ...
      if (raw) {
        ...
        if (/^05\d{8}$/.test(c)) norm='+966'+c.substring(1);
        else if (/^5\d{8}$/.test(c)) norm='+966'+c;
        else if (/^9665\d{8}$/.test(c)) norm='+'+c;
        else if (/^\+9665\d{8}$/.test(c)) norm=c;
        else return res.status(400).json({ error: 'رقم الجوال غير صحيح: 05xxxxxxxx' });
      }
      oldValues.phone=target.phone; newValues.phone=norm; updates.phone=norm;
    }
    if (Object.keys(updates).length===0) return res.status(400).json({ error: 'لا توجد حقول للتعديل' });
...
router.post('/users/:id/reset-password', requireHospitalAdmin as any, requirePermission('USER_MANAGE_ORG') as any, async (req, res) => {
  try {
    const orgId = req.user!.organization_id;
    const target = await prisma.user.findUnique({ where: { id: req.params.id as string }, include: { role: true } });
    if (!target || target.organization_id !== orgId) return res.status(404).json({ error: 'User not found in your organization' });
    ...
    if (!new_password || new_password.length<8 || !/[A-Za-z]/.test(new_password) || !/\d/.test(new_password)) return res.status(400).json({ error: 'كلمة المرور ضعيفة: 8+ حروف وأرقام' });
...
router.get('/users/:id', requireHospitalAdmin as any, requirePermission('USER_MANAGE_ORG') as any, async (req, res) => {
  try {
    const orgId = req.user!.organization_id;
    const u=await prisma.user.findUnique({ where: { id: req.params.id as string }, include: { role: true, organization: true } });
    if (!u || u.organization_id !== orgId) return res.status(404).json({ error: 'User not found' });
