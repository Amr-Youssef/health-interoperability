...
const router = Router();
const ALLOWED_EDITABLE_FIELDS = new Set<string>(ALLOWED_SELF_SERVICE_FIELDS as unknown as string[]);
...
router.get('/me', async (req: Request, res: Response) => {
  try {
    ...
    const identifiers = (patient.identifiers || []).map((i: any) => ({ type: i.type, value: i.value, system: i.system, isActive: i.is_active }));
  ...
  } catch (error) { console.error('Error fetching patient personal data:', error); res.status(500).json({ error: 'Internal server error while fetching personal data' }); }
});
...
router.patch('/me', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Error updating patient personal data:', error); res.status(500).json({ error: 'Internal server error while updating personal data' });
  }
});
