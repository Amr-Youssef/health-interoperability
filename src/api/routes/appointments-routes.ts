...
function consentTypeForAppointment(t: string): string {
  if (t === 'EMERGENCY') return 'EMERGENCY_OVERRIDE';
  if (t === 'REFERRAL') return 'CLUSTER_ONLY';
  if (t === 'CHRONIC') return 'OPT_IN_FULL';
  return 'EXPLICIT_PER_ENCOUNTER';
}

function expiresForType(t: string): Date | null {
  if (t === 'CHRONIC') return null;
  if (t === 'EMERGENCY') return new Date(Date.now() + 24*60*60*1000);
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!['ROUTINE','EMERGENCY','REFERRAL','CHRONIC'].includes(type)) return res.status(400).json({ error: 'appointment_type must be ROUTINE/EMERGENCY/REFERRAL/CHRONIC' });
    ...
    if (org.status !== 'ACTIVE') return res.status(403).json({ error: `Organization not active: ${org.status}` });
    if (!scheduled_start) return res.status(400).json({ error: 'scheduled_start required' });
    const start = new Date(scheduled_start);
    if (isNaN(start.getTime())) return res.status(400).json({ error: 'Invalid scheduled_start' });
    if (type !== 'EMERGENCY' && start <= new Date()) return res.status(400).json({ error: 'لا يمكن حجز موعد في الماضي' });
    if (type !== 'EMERGENCY') {
      ...
      if (isFriday || isSaturday) return res.status(400).json({ error: 'الجمعة والسبت عطلة - اختر الأحد إلى الخميس' });
      const hour = Number(new Date(start).toLocaleString('en-US', { timeZone: 'Asia/Riyadh', hour: '2-digit', hour12: false }));
      if (hour < 8 || hour >= 17) return res.status(400).json({ error: 'وقت الموعد خارج ساعات العمل (08:00-17:00 Asia/Riyadh)' });
    }
    ...
    if (isPatient) {
      if (!user.patient_profile_id) return res.status(400).json({ error: 'Patient profile missing' });
      ...
      if (!req.body.patient_id) return res.status(400).json({ error: 'patient_id required for clinician booking' });
      patientId = req.body.patient_id;
      const pat = await prisma.patient.findFirst({ where: { OR: [{ id: patientId }, { internal_id: patientId }] } });
      if (!pat) return res.status(404).json({ error: 'Patient not found' });
      patientId = pat.internal_id;
    } else {
      if (!req.body.patient_id) return res.status(400).json({ error: 'patient_id required' });
      patientId = req.body.patient_id;
    }
    if (clinician_id) {
      const clin = await prisma.user.findUnique({ where: { id: clinician_id }, include: { role: true } });
      if (!clin) return res.status(404).json({ error: 'الطبيب المفضل غير موجود' });
      if (clin.role?.role_code !== 'CLINICIAN') return res.status(400).json({ error: 'المستخدم المحدد ليس طبيب (CLINICIAN)' });
      if (clin.organization_id !== org.id) return res.status(403).json({ error: 'الطبيب لا يتبع نفس المنشأة - الرغبة مرفوضة' });
      if (!clin.is_active) return res.status(403).json({ error: 'حساب الطبيب غير نشط' });
    }
    ...
    if (type === 'EMERGENCY') {
      ...
      try {
        ...
        if (patientRow) {
          ...
          if (!existingLink) await prisma.patientOrganization.create({ data: { patient_id: patientRow.id, organization_id: org.id, relationship_type: 'emergency', active: true } });
          else if (!existingLink.active) await prisma.patientOrganization.update({ where: { id: existingLink.id }, data: { active: true } });
        }
      } catch {}
    }
...
router.get('/my', async (req: Request, res: Response) => {
  try {
    ...
    if (!patientId && user.role?.role_code === 'PATIENT') return res.json([]);
    const where: any = user.role?.role_code === 'PATIENT' ? { patient_id: patientId } : { organization_id: user.organization_id };
...
router.get('/organization', async (req: Request, res: Response) => {
  try {
    const user: any = (req as any).user;
    if (!['HOSPITAL_ADMIN','CLINICIAN','MOH_ADMIN','SYS_ADMIN'].includes(user.role?.role_code)) return res.status(403).json({ error: 'Hospital staff only' });
    const orgId = (req.query.organization_id as string) || user.organization_id;
    if (user.role?.role_code === 'CLINICIAN' || user.role?.role_code === 'HOSPITAL_ADMIN') {
      if (orgId !== user.organization_id) return res.status(403).json({ error: 'Cross-org denied' });
    }
...
router.patch('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    if (!['proposed','booked','arrived','fulfilled','cancelled','no_show'].includes(status)) return res.status(400).json({ error: 'Invalid status' });
    if (clinician_id) {
      const clin = await prisma.user.findUnique({ where: { id: clinician_id }, include: { role: true } });
    ...
    if (user.role?.role_code === 'CLINICIAN' || user.role?.role_code === 'HOSPITAL_ADMIN') {
      ...
      if (appt.patient_id !== pid) return res.status(403).json({ error: 'Not your appointment' });
      if (!['cancelled'].includes(status)) return res.status(403).json({ error: 'Patient can only cancel' });
    }
    ...
    if (targetClinicianId) {
      ...
      if (!targetClin || targetClin.organization_id !== appt.organization_id) return res.status(403).json({ error: 'الطبيب لا يتبع نفس المنشأة' });
    }
    ...
    if (status === 'booked' && appt.consent) {
      ...
      try {
        ...
        if (patientRow) {
          ...
          if (!existingLink) await prisma.patientOrganization.create({ data: { patient_id: patientRow.id, organization_id: appt.organization_id, relationship_type: appt.appointment_type === 'REFERRAL' ? 'referred' : appt.appointment_type === 'EMERGENCY' ? 'emergency' : 'registered', active: true } });
          else if (!existingLink.active) await prisma.patientOrganization.update({ where: { id: existingLink.id }, data: { active: true } });
        }
      } catch {}
    }
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const appt = await prisma.appointment.findUnique({ where: { id: req.params.id as string }, include: { organization: true, consent: true, clinician: true } });
    if (!appt) return res.status(404).json({ error: 'Not found' });
    const user: any = (req as any).user;
    if (user.role?.role_code === 'PATIENT') {
      const pat = user.patient_profile_id ? await prisma.patient.findUnique({ where: { id: user.patient_profile_id } }) : null;
      const pid = pat?.internal_id || user.patient_profile_id;
      if (appt.patient_id !== pid) return res.status(403).json({ error: 'Forbidden' });
