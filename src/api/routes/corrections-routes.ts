...
router.post('/:type/:id/request', async (req: Request, res: Response) => {
  try {
    const user: any = (req as any).user;
    const role = user.role?.role_code;
    if (!['CLINICIAN','HOSPITAL_ADMIN'].includes(role)) return res.status(403).json({ error: 'Only CLINICIAN/HOSPITAL_ADMIN can request correction' });
    const type = req.params.type as string;
    const id = req.params.id as string;
    const { reason } = req.body;
    if (!allowedTypes.includes(type)) return res.status(400).json({ error: 'Invalid type' });
    if (!reason || String(reason).trim().length < 5) return res.status(400).json({ error: 'correction reason required (5+ chars)' });
    const modelMap: any = { encounter: prisma.encounter, condition: prisma.condition, observation: prisma.observation, medication: prisma.medicationRequest };
    const model = modelMap[type];
    const record: any = await model.findUnique({ where: { id } });
...
router.post('/:type/:id/approve', async (req: Request, res: Response) => {
  try {
    const user: any = (req as any).user;
    const role = user.role?.role_code;
    if (!['HOSPITAL_ADMIN','MOH_ADMIN','SYS_ADMIN'].includes(role)) return res.status(403).json({ error: 'Only HOSPITAL_ADMIN/MOH can approve' });
    const type = req.params.type as string;
    const id = req.params.id as string;
    const { decision, reason } = req.body;
    if (!['CORRECTED','REDACTED','ENTERED_IN_ERROR','REJECTED'].includes(decision)) return res.status(400).json({ error: 'decision must be CORRECTED/REDACTED/ENTERED_IN_ERROR/REJECTED' });
    const modelMap: any = { encounter: prisma.encounter, condition: prisma.condition, observation: prisma.observation, medication: prisma.medicationRequest };
    const model = modelMap[type];
    if (!model) return res.status(400).json({ error: 'Invalid type' });
    const record: any = await model.findUnique({ where: { id } });
    if (!record) return res.status(404).json({ error: 'Not found' });
    if (record.correction_status !== 'CORRECTION_REQUEST') return res.status(400).json({ error: 'No pending correction request' });
...
router.get('/pending', async (req: Request, res: Response) => {
  try {
    const user: any = (req as any).user;
    const role = user.role?.role_code;
    if (!['HOSPITAL_ADMIN','MOH_ADMIN','SYS_ADMIN','MOH_AUDITOR'].includes(role)) return res.status(403).json({ error: 'Not authorized' });
    const where: any = { correction_status: 'CORRECTION_REQUEST' };
