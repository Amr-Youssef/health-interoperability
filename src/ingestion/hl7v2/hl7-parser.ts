...
export class Hl7v2MessageParser {
  ...
  extractPatient(parsed: ParsedHl7Message): {
    ...
    if (!pid) throw new Error('PID segment missing in HL7 v2 message');
    ...
    if (!mrn && nationalId) mrn = `MRN-${nationalId}`;
    ...
    let givenNameAr: string | undefined;
    let familyNameAr: string | undefined;
