...
export class PatientReportedHealthService {
  ...
  // === PROFILE MANAGEMENT ===
  async createOrUpdateProfile(patientId: string, data: Partial<PatientProfileData>): Promise<PatientProfileData> {
    ...
    await this.auditChain.recordEvent(
      ...
      'PatientProfile',
      profile.id,
      `Patient ${existing ? 'updated' : 'created'} profile`
    );

    return this.mapProfileToData(profile);
  }

  async getProfile(patientId: string): Promise<PatientProfileData | null> {
  ...
  // === ALLERGIES ===
  async createAllergy(patientId: string, data: PatientReportedAllergyData): Promise<PatientReportedAllergyData> {
  ...
  async getAllergies(patientId: string): Promise<PatientReportedAllergyData[]> {
  ...
  async updateAllergy(allergyId: string, patientId: string, data: Partial<PatientReportedAllergyData>): Promise<PatientReportedAllergyData> {
  ...
  // === MEDICATIONS ===
  async createMedication(patientId: string, data: PatientReportedMedicationData): Promise<PatientReportedMedicationData> {
  ...
  async getMedications(patientId: string): Promise<PatientReportedMedicationData[]> {
  ...
  async updateMedication(medicationId: string, patientId: string, data: Partial<PatientReportedMedicationData>): Promise<PatientReportedMedicationData> {
  ...
  async deleteMedication(medicationId: string, patientId: string): Promise<void> {
    const medication = await this.prisma.patientReportedMedication.findUnique({ where: { id: medicationId } });
    if (!medication || medication.patient_id !== patientId) {
      throw new Error('Medication not found or unauthorized access');
    }
  ...
  // === CONDITIONS ===
  async createCondition(patientId: string, data: PatientReportedConditionData): Promise<PatientReportedConditionData> {
  ...
  async getConditions(patientId: string): Promise<PatientReportedConditionData[]> {
  ...
  async updateCondition(conditionId: string, patientId: string, data: Partial<PatientReportedConditionData>): Promise<PatientReportedConditionData> {
  ...
  async deleteCondition(conditionId: string, patientId: string): Promise<void> {
    const condition = await this.prisma.patientReportedCondition.findUnique({ where: { id: conditionId } });
    if (!condition || condition.patient_id !== patientId) {
      throw new Error('Condition not found or unauthorized access');
    }
  ...
  // === PROCEDURES ===
  async createProcedure(patientId: string, data: PatientReportedProcedureData): Promise<PatientReportedProcedureData> {
  ...
  async getProcedures(patientId: string): Promise<PatientReportedProcedureData[]> {
  ...
  async updateProcedure(procedureId: string, patientId: string, data: Partial<PatientReportedProcedureData>): Promise<PatientReportedProcedureData> {
  ...
  async deleteProcedure(procedureId: string, patientId: string): Promise<void> {
    const procedure = await this.prisma.patientReportedProcedure.findUnique({ where: { id: procedureId } });
    if (!procedure || procedure.patient_id !== patientId) {
      throw new Error('Procedure not found or unauthorized access');
    }
  ...
  // === FAMILY HISTORY ===
  async createFamilyMember(patientId: string, data: FamilyMemberData): Promise<FamilyMemberData> {
  ...
  async getFamilyMembers(patientId: string): Promise<FamilyMemberData[]> {
  ...
  async updateFamilyMember(memberId: string, patientId: string, data: Partial<FamilyMemberData>): Promise<FamilyMemberData> {
  ...
  async deleteFamilyMember(memberId: string, patientId: string): Promise<void> {
    const member = await this.prisma.familyMember.findUnique({ where: { id: memberId } });
    if (!member || member.patient_id !== patientId) {
      throw new Error('Family member record not found or unauthorized access');
    }
  ...
  // === SOCIAL HISTORY ===
  async createOrUpdateSocialHistory(patientId: string, data: Partial<PatientReportedSocialHistoryData>): Promise<PatientReportedSocialHistoryData> {
    ...
    await this.auditChain.recordEvent(
      'INGEST',
      patientId,
      'PatientReportedSocialHistory',
      socialHistory.id,
      `Patient ${existing ? 'updated' : 'created'} social history`
    );

    return this.mapSocialHistoryToData(socialHistory);
  }

  async getSocialHistory(patientId: string): Promise<PatientReportedSocialHistoryData | null> {
  ...
  // === VITAL OBSERVATIONS ===
  async createVitalObservation(patientId: string, data: PatientReportedVitalObservationData): Promise<PatientReportedVitalObservationData> {
  ...
  async getVitalObservations(patientId: string, observationType?: string): Promise<PatientReportedVitalObservationData[]> {
  ...
  async deleteVitalObservation(vitalId: string, patientId: string): Promise<void> {
  ...
  // === DOCUMENTS ===
  async createDocument(patientId: string, data: PatientUploadedDocumentData): Promise<PatientUploadedDocumentData> {
  ...
  async getDocuments(patientId: string, category?: string): Promise<PatientUploadedDocumentData[]> {
  ...
  async updateDocumentStatus(documentId: string, patientId: string, processingStatus: string, verificationStatus?: string): Promise<PatientUploadedDocumentData> {
  ...
  async deleteDocument(documentId: string, patientId: string): Promise<void> {
    const document = await this.prisma.patientUploadedDocument.findUnique({ where: { id: documentId } });
    if (!document || document.patient_id !== patientId) {
      throw new Error('Document not found or unauthorized access');
    }
  ...
  async getPatientHealthProfile(patientId: string): Promise<PatientHealthProfile> {
    return {
  ...
  private mapProfileToData(profile: any): PatientProfileData {
    return {
  ...
  private mapAllergyToData(allergy: any): PatientReportedAllergyData {
    return {
  ...
  private mapMedicationToData(medication: any): PatientReportedMedicationData {
    return {
  ...
  private mapConditionToData(condition: any): PatientReportedConditionData {
    return {
  ...
  private mapProcedureToData(procedure: any): PatientReportedProcedureData {
    return {
  ...
  private mapFamilyMemberToData(member: any): FamilyMemberData {
    return {
  ...
  private mapSocialHistoryToData(socialHistory: any): PatientReportedSocialHistoryData {
    return {
  ...
  private mapVitalObservationToData(vital: any): PatientReportedVitalObservationData {
    return {
  ...
  private mapDocumentToData(document: any): PatientUploadedDocumentData {
    return {
