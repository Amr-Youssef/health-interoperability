...
export class PrismaMpiService {
  ...
  private mapIdentityToDomain(dbId: any): InternalPatientIdentity {
    return {
      internalPatientId: dbId.id,
      status: dbId.status as any,
      mergedInto: dbId.merged_into,
      linkedIdentifiers: dbId.mpi_identifiers.map((i: any) => ({
        value: i.value,
        type: i.type as any,
        system: i.system,
      ...
      matchHistory: dbId.match_history.map((m: any) => ({
        matchedIdentifier: m.matched_identifier,
        matchStrategy: m.match_strategy as any,
        confidence: m.confidence,
        matchedAt: m.matched_at.toISOString(),
        matchedBy: m.matched_by,
        decision: m.decision as any,
        details: m.details
      })),
  ...
  private async linkNewIdentifiers(
    identity: any,
    ...
    if (input.mrn) {
      const exists = identity.mpi_identifiers.some(
        (id: any) => id.type === 'MRN' && id.value === input.mrn && id.source_system_id === input.sourceSystemId
      );
      if (!exists) {
        newIdentifiers.push({
          value: input.mrn.trim(),
    ...
    if (input.nationalId) {
      const exists = identity.mpi_identifiers.some(
        (id: any) => id.type === 'NID' && id.value === input.nationalId
      );
      if (!exists) {
        newIdentifiers.push({
          value: input.nationalId.trim(),
  ...
  async mergePatientIdentities(survivorId: string, obsoleteId: string, reason: string, adminUser: string = 'HIE_ADMIN') {
    ...
    if (!survivor) throw new Error(`Survivor identity not found.`);
    if (!obsolete) throw new Error(`Obsolete identity not found.`);

    const timestamp = new Date();

    const newIdentifiersForSurvivor = obsolete.mpi_identifiers.filter(obsId => 
      !survivor.mpi_identifiers.some(s => s.type === obsId.type && s.value === obsId.value && s.source_system_id === obsId.source_system_id)
    ).map(obsId => ({
      value: obsId.value,
      type: obsId.type,
      system: obsId.system,
  ...
  async unmergePatientIdentities(survivorId: string, obsoleteId: string, reason: string, adminUser: string = 'HIE_ADMIN') {
