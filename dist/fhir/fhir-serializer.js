export class FhirR4Serializer {
     ...
     */
    serializePatient(patient, opts) {
        const mask = (v) => v.length >= 7 ? v[0] + 'XXXXX' + v.slice(-4) : 'XXXX';
        ...
        return {
            ...
            id: patient.internalIdUuid || patient.internalId,
            meta: {
     ...
     */
    serializeLongitudinalBundle(record) {
        const entries = [];
        // Patient
        entries.push({
     ...
     */
    serializeAllergyIntolerance(allergy) {
        const codings = [];
