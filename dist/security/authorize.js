...
export const ROLE_PERMISSIONS = {
    ...
    MOH_ADMIN: ['ORG_APPROVE', 'POLICY_MANAGE', 'QUALITY_MONITOR', 'USER_MANAGE_NATIONAL', 'ROLE_ASSIGN_NATIONAL', 'AUDIT_READ_CENTRAL', 'ANALYTICS_READ_NATIONAL', 'CLINICAL_READ_ALL', 'FHIR_READ_ALL', 'CONSENT_OVERRIDE', 'BREAK_GLASS_EXECUTE', 'EXPORT_BULK_ANONYMIZED', 'PATIENT_READ_ALL'],
...
export async function getUserPermissions(userId, roleCode) {
    ...
    try {
        const dbRolePerms = await prisma.rolePermission.findMany({ where: { role: { role_code: roleCode } } });
        for (const rp of dbRolePerms) {
            if (rp.granted)
                perms.add(rp.permission_code);
            else
                perms.delete(rp.permission_code);
        }
...
export function enforceOrgScope() {
    return (req, res, next) => {
        ...
        if (roleCode === 'MOH_ADMIN' || roleCode === 'MOH_AUDITOR' || roleCode === 'SYS_ADMIN')
            return next();
        const targetOrg = req.params.orgId || req.params.id || req.body?.organization_id || req.query.orgId;
