export class CdsHooksEngine {
    ...
    async evaluateDraftPrescription(draft) {
        ...
        for (const alg of allergies) {
            if (alg.clinicalStatus === 'active') {
                const isPenicillinAllergy = alg.substanceCode?.snomedCode === '764146007' ||
                ...
                const isDraftBetaLactam = drugNameUpper.includes('PENICILLIN') ||
                ...
                const isSulfaAllergy = alg.substanceCode?.snomedCode === '294509007' ||
                    alg.substanceText?.toLowerCase().includes('sulfa') ||
                    alg.substanceTextAr?.includes('سلفا');
                const isDraftSulfa = drugNameUpper.includes('SULFA') ||
     ...
     */
    async evaluateMedicationSafety(patientId) {
        ...
        const hasMetformin = activeMeds.some(m => m.medication?.code?.sfdaCode === '0628500100101' ||
        ...
        const hasDiabetes = record.conditions.some(c => c.code?.icd10amCode === 'E11' ||
            c.code?.snomedCode === '44054006' ||
            c.code?.sourceCode?.includes('سكري'));
        if (hasDiabetes) {
            const immunizations = record.immunizations || [];
            const hasFluVax = immunizations.some(i => i.vaccineCode?.cvxCode === '158' ||
...
//# sourceMappingURL=cds-engine.js.map