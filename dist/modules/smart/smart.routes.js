...
export function createSmartRoutes(engine) {
    const router = Router();
    const DISABLE_RATE_LIMIT = process.env.DISABLE_RATE_LIMIT === 'true' && process.env.NODE_ENV !== 'production';
    const oauthLimiter = DISABLE_RATE_LIMIT ? ((req, _res, next) => next()) : rateLimit({ windowMs: 15 * 60 * 1000, max: 30, standardHeaders: true, legacyHeaders: false, message: { error: 'محاولات OAuth كثيرة - حاول بعد 15 دقيقة' } });
    ...
    router.post('/oauth/introspect', oauthLimiter, async (req, res) => {
