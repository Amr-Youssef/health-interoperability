...
export function createCdsRoutes(engine) {
    const router = Router();
    router.get('/cds/patient/:id/safety-alerts', verifyToken, requirePermission('CLINICAL_READ_ORG', 'CLINICAL_READ_ALL'), async (req, res) => {
        try {
    ...
    router.post('/cds/evaluate-draft-prescription', verifyToken, requirePermission('CLINICAL_WRITE_ORG', 'CLINICAL_READ_ORG'), async (req, res) => {
        try {
