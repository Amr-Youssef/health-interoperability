export class TransformEngine {
    ...
    registerDefaultTransforms() {
        ...
        this.transforms.set('gender_normalize', (val) => {
            if (!val)
                return 'unknown';
            const s = String(val).trim().toLowerCase();
            if (s === 'ذ' || s === 'ذكر' || s === 'm' || s === 'male' || s === '1')
                return 'male';
            if (s === 'أ' || s === 'أنثى' || s === 'f' || s === 'female' || s === '2')
                return 'female';
            if (s === 'other' || s === 'آخر')
                return 'other';
            return 'unknown';
        ...
        this.transforms.set('date_normalize', (val) => {
            if (!val)
                return undefined;
        ...
        this.transforms.set('datetime_normalize', (val) => {
            if (!val)
                return undefined;
            const s = String(val).trim();
        ...
        this.transforms.set('encounter_type_map', (val) => {
            if (!val)
                return 'outpatient';
            const s = String(val).trim().toUpperCase();
            if (s === 'ع' || s === 'OPD' || s === 'AMB' || s === 'AMBULATORY' || s === 'عيادة')
                return 'outpatient';
            if (s === 'ط' || s === 'EMR' || s === 'EMERGENCY' || s === 'طوارئ')
                return 'emergency';
            if (s === 'د' || s === 'INP' || s === 'INPATIENT' || s === 'تنويم')
                return 'inpatient';
            if (s === 'افتراضي' || s === 'VR' || s === 'VIRTUAL')
                return 'virtual';
            return 'outpatient';
        });
        // 5. Diagnosis Rank Mapping
        this.transforms.set('diagnosis_rank_map', (val) => {
            if (!val)
                return 'secondary';
            const s = String(val).trim().toLowerCase();
            if (s === 'ر' || s === 'رئيسي' || s === '1' || s === 'primary' || s === 'main')
                return 'primary';
            return 'secondary';
        });
        // 6. Number / Decimal parser
        this.transforms.set('number_parse', (val) => {
        ...
        this.transforms.set('arabic_clean', (val) => {
            if (!val)
                return '';
            return String(val)
                .trim()
    ...
    transform(ruleName, value, params) {
