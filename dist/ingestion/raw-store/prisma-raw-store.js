...
export class PrismaRawStore {
    ...
    async recordPipelineTrace(id, trace) {
        ...
        if (trace.validationScore !== undefined)
            data.validation_score = trace.validationScore;
        if (trace.validationDecision !== undefined)
            data.validation_decision = trace.validationDecision;
        if (trace.validationIssuesCount !== undefined)
            data.validation_issues_count = trace.validationIssuesCount;
        if (trace.mappingVersion !== undefined)
            data.mapping_version = trace.mappingVersion;
        if (trace.mappingConfigId !== undefined)
            data.mapping_config_id = trace.mappingConfigId;
        if (trace.terminologySummary !== undefined)
            data.terminology_summary = trace.terminologySummary ? JSON.stringify(trace.terminologySummary) : null;
        if (trace.mpiStrategy !== undefined)
            data.mpi_strategy = trace.mpiStrategy;
        if (trace.mpiConfidence !== undefined)
            data.mpi_confidence = trace.mpiConfidence;
        if (trace.mpiIdentityId !== undefined)
            data.mpi_identity_id = trace.mpiIdentityId;
