...
const ALLOWED = new Set(ALLOWED_SELF_SERVICE_FIELDS);
function normalizeSaudiPhone(raw) {
    const c = raw.replace(/[\s\-\(\)]/g, '');
    if (/^05\d{8}$/.test(c))
        return '+966' + c.substring(1);
    if (/^5\d{8}$/.test(c))
        return '+966' + c;
    if (/^9665\d{8}$/.test(c))
        return '+' + c;
...
export function validateSelfServicePayload(body) {
    ...
    if (body.phone !== undefined) {
        ...
        if (!np)
            return { valid: false, error: 'رقم الجوال غير صحيح: يجب أن يكون رقم سعودي يبدأ بـ 05 أو +9665 (مثال: 0555123456)' };
        n.phone = np;
    }
    if (body.email !== undefined) {
        ...
        if (raw === '')
            ...
            n.email = raw.toLowerCase();
        }
    }
    if (body.preferredFirstName !== undefined) {
        const v = String(body.preferredFirstName).trim();
        if (v && (v.length < 2 || v.length > 50))
            return { valid: false, error: 'الاسم المفضل يجب أن يكون بين 2 و 50 حرفاً' };
        n.preferredFirstName = v || null;
    }
    if (body.preferredLastName !== undefined) {
        const v = String(body.preferredLastName).trim();
        if (v && (v.length < 2 || v.length > 50))
            return { valid: false, error: 'اسم العائلة المفضل يجب أن يكون بين 2 و 50 حرفاً' };
        n.preferredLastName = v || null;
    }
    if (body.preferredLanguage !== undefined) {
        const v = String(body.preferredLanguage).trim().toLowerCase();
        if (!['ar', 'en'].includes(v))
            return { valid: false, error: 'اللغة المفضلة يجب أن تكون ar أو en' };
        n.preferredLanguage = v;
    }
    if (body.emergencyContactName !== undefined) {
        const v = String(body.emergencyContactName).trim();
        if (v && (v.length < 2 || v.length > 80))
            return { valid: false, error: 'اسم جهة الطوارئ يجب أن يكون بين 2 و 80 حرفاً' };
        n.emergencyContactName = v || null;
    }
    if (body.emergencyContactPhone !== undefined) {
        const raw = String(body.emergencyContactPhone).trim();
        if (raw === '')
            n.emergencyContactPhone = null;
        else {
            const np = normalizeSaudiPhone(raw);
            if (!np)
                return { valid: false, error: 'رقم طوارئ غير صحيح: يجب أن يكون رقم سعودي' };
            n.emergencyContactPhone = np;
        }
    }
    if (body.emergencyContactRelationship !== undefined) {
        let v = String(body.emergencyContactRelationship).trim().toUpperCase();
        if (v === 'FATHER' || v === 'MOTHER')
            v = 'PARENT';
        const allowed = ['PARENT', 'SPOUSE', 'SIBLING', 'CHILD', 'FRIEND', 'OTHER'];
        if (v && !allowed.includes(v))
            return { valid: false, error: 'العلاقة غير صالحة' };
        n.emergencyContactRelationship = v || null;
    }
    if (body.addressLine !== undefined) {
        const v = String(body.addressLine).trim();
        if (v && (v.length < 3 || v.length > 200))
            return { valid: false, error: 'العنوان يجب أن يكون بين 3 و 200 حرفاً' };
        n.addressLine = v || null;
    }
    if (body.addressCity !== undefined) {
        const v = String(body.addressCity).trim();
        if (v && (v.length < 2 || v.length > 50))
            return { valid: false, error: 'المدينة يجب أن تكون بين 2 و 50 حرفاً' };
        n.addressCity = v || null;
    }
    if (body.addressDistrict !== undefined) {
        const v = String(body.addressDistrict).trim();
        if (v && (v.length < 2 || v.length > 50))
            return { valid: false, error: 'الحي يجب أن يكون بين 2 و 50 حرفاً' };
        n.addressDistrict = v || null;
    }
    if (body.addressPostalCode !== undefined) {
        const v = String(body.addressPostalCode).trim();
        if (v && !/^\d{5}$/.test(v))
            return { valid: false, error: 'الرمز البريدي يجب أن يكون 5 أرقام (مثال: 12345)' };
        n.addressPostalCode = v || null;
    }
    if (body.notes !== undefined) {
        const v = String(body.notes).trim();
        if (v.length > 500)
            return { valid: false, error: 'الملاحظات يجب ألا تتجاوز 500 حرف' };
        n.notes = v || null;
    }
    return { valid: true, normalized: n };
}
export async function getPatientSelfView(patientId, userId) {
...
export async function updatePatientSelfData(patientId, userId, organizationId, body) {
    ...
    if (data.email) {
        ...
        if (dup) {
            const e = new Error('البريد الإلكتروني مسجل لمستخدم آخر');
            e.status = 400;
            throw e;
        }
    }
    ...
    const mapProfile = { preferredFirstName: 'preferred_first_name', preferredLastName: 'preferred_last_name', preferredLanguage: 'preferred_language', emergencyContactName: 'emergency_contact_name', emergencyContactPhone: 'emergency_contact_phone', emergencyContactRelationship: 'emergency_contact_relationship', addressLine: 'address_line', addressCity: 'address_city', addressDistrict: 'address_district', addressPostalCode: 'address_postal_code', notes: 'notes' };
    const profileFields = {};
    for (const [camel, snake] of Object.entries(mapProfile))
        if (data[camel] !== undefined) {
            profileFields[snake] = data[camel];
            oldValues[camel] = existingProfile?.[snake] ?? null;
            newValues[camel] = data[camel];
        }
    let updatedProfile = existingProfile;
    if (Object.keys(profileFields).length) {
        if (existingProfile)
            updatedProfile = await prisma.patientProfile.update({ where: { patient_id: patientId }, data: { ...profileFields, updated_at: new Date() } });
        else
            updatedProfile = await prisma.patientProfile.create({ data: { patient_id: patientId, ...profileFields, source: 'PATIENT', verification_status: 'SELF_REPORTED', recorded_at: new Date() } });
    }
    try {
