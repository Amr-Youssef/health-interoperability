...
export function createPlatformApp() {
    ...
    app.post('/api/hospitals/onboard', verifyToken, requirePermission('ORG_MANAGE_ALL'), async (req, res) => {
        try {
            ...
            await prisma.auditLog.create({ data: { entity_type: 'Organization', entity_id: org.id, action: 'HOSPITAL_ONBOARDED_PENDING', actor_id: req.user?.id, organization_id: org.id, new_values: JSON.stringify({ hospitalName: org.organization_name, region }), details: `SYS_ADMIN onboarded ${org.organization_name_ar} -> PENDING_APPROVAL awaiting MOH` } }).catch(() => { });
