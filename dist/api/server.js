...
export function createPlatformApp() {
    ...
    if (DISABLE_RATE_LIMIT)
        console.log('[DEV] Rate limiting disabled via DISABLE_RATE_LIMIT=true (non-production only)');
    ...
    app.use('/fhir', createFhirRoutes(canonicalStore, fhirSerializer, engine));
    app.use('/api', createPlatformRoutes(engine));
    app.use('/api', createNphiesRoutes(canonicalStore, engine));
    ...
    app.use('/api', createPatientsRoutes(canonicalStore));
    ...
    app.post('/api/hospitals/:id/ingest', verifyToken, requirePermission('IMPORT_EXECUTE_ORG'), async (req, res) => {
        try {
            ...
            await prismaInstance.auditLog.create({ data: { entity_type: 'DataIngest', entity_id: sourceRecordId, action: 'INGEST_CUSTOM', actor_id: req.user?.id, organization_id: targetId, new_values: JSON.stringify({ entityType, sourceRecordId }), details: `Custom ingest to ${org.organization_name_ar}` } }).catch(() => { });
    ...
    app.post('/api/ingest/file', verifyToken, requirePermission('IMPORT_EXECUTE_ORG'), async (req, res) => {
        try {
            ...
            await prismaInstance.auditLog.create({ data: { entity_type: 'DataImport', entity_id: fileName, action: 'FILE_INGEST', actor_id: req.user?.id, organization_id: resolvedSystemId, new_values: JSON.stringify({ fileName, format: result.format }), details: `File ingest ${fileName} via ${resolvedSystemId}` } }).catch(() => { });
            res.json({ success: true, result });
        }
        catch (err) {
            res.status(500).json({ success: false, error: err.message });
