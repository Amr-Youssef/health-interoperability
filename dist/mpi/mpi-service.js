...
export class MasterPatientIndexService {
    ...
    async resolvePatientIdentity(input) {
        ...
        if (input.nationalId && input.nationalId.trim()) {
            ...
            for (const identity of this.identities.values()) {
                if (identity.status !== 'ACTIVE')
                    continue;
                const hasMatch = identity.linkedIdentifiers.some(id => id.type === 'NID' && id.value.trim() === nid);
                if (hasMatch) {
        ...
        if (input.iqamaNo && input.iqamaNo.trim()) {
            ...
            for (const identity of this.identities.values()) {
                if (identity.status !== 'ACTIVE')
                    continue;
                const hasMatch = identity.linkedIdentifiers.some(id => id.type === 'IQAMA' && id.value.trim() === iqama);
                if (hasMatch) {
    ...
    linkNewIdentifiers(identity, input, strategy, confidence, timestamp) {
        // Add MRN if not present
        if (input.mrn) {
            const exists = identity.linkedIdentifiers.some(id => id.type === 'MRN' && id.value === input.mrn && id.sourceSystemId === input.sourceSystemId);
            if (!exists) {
        ...
        if (input.nationalId) {
            const exists = identity.linkedIdentifiers.some(id => id.type === 'NID' && id.value === input.nationalId);
            if (!exists) {
                identity.linkedIdentifiers.push({
                    value: input.nationalId.trim(),
                    type: 'NID',
    ...
    async findByIdentifier(type, value) {
        for (const identity of this.identities.values()) {
            if (identity.status !== 'ACTIVE')
                continue;
            const match = identity.linkedIdentifiers.some(id => id.type.toLowerCase() === type.toLowerCase() && id.value === value);
            if (match)
                return { ...identity };
        }
        return null;
    ...
    async mergePatientIdentities(survivorId, obsoleteId, reason, adminUser = 'HIE_ADMIN') {
        ...
        if (!survivor)
            throw new Error(`Survivor identity [${survivorId}] not found.`);
        if (!obsolete)
            throw new Error(`Obsolete identity [${obsoleteId}] not found.`);
        ...
        for (const obsId of obsolete.linkedIdentifiers) {
            const alreadyPresent = survivor.linkedIdentifiers.some(s => s.type === obsId.type && s.value === obsId.value && s.sourceSystemId === obsId.sourceSystemId);
            if (!alreadyPresent) {
...
//# sourceMappingURL=mpi-service.js.map