  ...
  window.SR = {
    ...
    async reload() {
      try {
        ...
        var vitals = asList(p.vitalObservations || p.vitals);
        this.render('sr-vit-list', vitals.map(function (v) { return row(v.observationType, (v.valueQuantity != null ? v.valueQuantity : (v.systolic ? v.systolic + '/' + v.diastolic : '')), pickId(v), 'vital'); }).join('') || '<small>لا توجد قياسات</small>');
        try { if (window.VitalsCharts) VitalsCharts.render('sr-vit-charts', vitals); } catch (e) {}
